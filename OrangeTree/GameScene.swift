...
//  OrangeTree
//
//  Created by Ricardo Rodriguez on 9/25/18.
...
class GameScene: SKScene {
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        // Get the location of where the touched ended
        let touch = touches.first!
        let location = touch.location(in: self)
        
        // Get the difference between the start and end point as a vector
        let dx = (touchStart.x - location.x) * 0.8
        let dy = (touchStart.y - location.y) * 0.8
